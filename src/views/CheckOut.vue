      ...
      <v-row>
        <v-col cols="7" style="padding: 60px 40px 0px 130px">
          <v-card elevation="0" color="white" class="pa-5 w-100">
            ...
            <v-card-actions class="d-flex justify-space-between px-0 mb-4">
              <v-btn
                color="#6200ff"
                variant="elevated"
                class="flex-grow-1"
            ...
            >
            <select class="w-100 border pa-3 rounded mt-2">
            ...
            <input
              type="text"
              placeholder="Address"
              class="border rounded pa-3 w-100 mt-2"
            />
            <input
              type="text"
              ...
              class="border rounded pa-3 w-100 mt-2"
            />
            <div class="d-flex align-center justify-lg-center ga-3 mt-2">
              <input
                type="text"
                placeholder="City"
                class="border rounded pa-3 w-33"
              />
              <select class="border pa-3 rounded w-33">
                <option v-for="country in countries" :key="country" value="">
                  {{ country }}
                </option>
              </select>
            ...
            <v-card-actions class="justify-end">
              <v-btn
                color="rgb(62 102 155)"
                variant="elevated"
                class="text-capitalize mt-3 text-white"
                @click="dialog = true"
              >
                Submit
              </v-btn>
          ...
          <div class="mt-5 border-t-md pt-2">
        ...
        <v-col cols="5">
          <v-card
            ...
            height="100%"
            style="padding: 60px 100px 0px 40px"
          >
            <v-card
              elevation="0"
            ...
            >
              <v-badge color="gray-darken-1" :content="item.quantity">
                <img
                  :src="item.thumbnail"
                  alt=""
                  width="70"
              ...
              <div class="title-parent flex-grow-1 line-clamp">
                <v-card-title
                  style="
                    font-size: 13px;
                    font-weight: bold;
                  ...
                  sale</v-card-title
                >
                <v-card-text style="font-size: 11px" class="text-grey-darken-1"
                  >Category:{{ item.category }}</v-card-text
                >
              </div>
              <v-card-text class="flex-grow-0 text-grey-darken-3">
                ${{ (item.price * item.quantity).toFixed(2) }}</v-card-text
              >
            </v-card>
            <v-divider length="100%" color="gray-darken-1"></v-divider>
            <v-divider length="100%" color="gray-darken-1"></v-divider>
            <v-card-text class="d-flex justify-space-between font-weight-bold"
              ><span>Total</span><span>${{ calcToPrice }}</span></v-card-text
            >
          </v-card>
        </v-col>
      </v-row>
